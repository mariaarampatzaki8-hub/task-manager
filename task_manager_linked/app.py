...
# -------------------- DB CONFIG: Render Postgres via pg8000 --------------------
...
if db_url:
    ...
    # 2) Βάλε driver pg8000
    if db_url.startswith("postgresql://"):
        db_url = db_url.replace("postgresql://", "postgresql+pg8000://", 1)

    # 3) Καθάρισε ΠΛΗΡΩΣ τυχόν ssl/sslmode από το query string
    ...
    db_url = urlunsplit((parts.scheme, parts.netloc, parts.path, clean_query, parts.fragment))

    app.config["SQLALCHEMY_DATABASE_URI"] = db_url
else:
    # Fallback σε SQLite για local
...
def admin_create_user():
    ...
    u = User(
        ...
        email=email or None, phone=phone or None, id_card=id_card or None,
        color=color or "#3273dc", is_admin=is_admin, must_change_password=True
    )
