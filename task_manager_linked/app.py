...
# ================= App & DB config (Render Postgres via pg8000) =================
...
def create_app():
    ...
    # Instance folder & sqlite fallback
    os.makedirs(app.instance_path, exist_ok=True)
    db_path = os.path.join(app.instance_path, "app_final.db")

    uri = os.environ.get("DATABASE_URL")
    if uri:
        # Heroku-style to official
        if uri.startswith("postgres://"):
            uri = uri.replace("postgres://", "postgresql://", 1)
        # Force pg8000 driver
        if uri.startswith("postgresql://"):
            uri = uri.replace("postgresql://", "postgresql+pg8000://", 1)
        # SSL for Render Postgres (no connect_args!)
...
class Task(db.Model):
    ...
    created_at = db.Column(db.DateTime, default=datetime.utcnow)
    due_date = db.Column(db.String(20), nullable=True) # απλά strings για απλά templates
...
def inject_nav_user():
    # Διαθέσιμο στα templates ως nav_user (ασφαλές για Flask 3)
...
def _init_db_and_seed_once():
    ...
    if not admin:
        admin = User(name="Admin", username="admin", is_admin=True, color="#3273dc")
        admin.set_password("admin123")
        db.session.add(admin)
        db.session.commit()
        app.logger.info("Seeded admin (admin/admin123)")
...
# ================= User: Dashboard / Tasks / Notes / Settings =================
@app.route("/dashboard")
@login_required
def dashboard():
    u = current_user()
...
def admin_delete_user(user_id):
    ...
    for n in u.notes.all():
        db.session.delete(n)
    # αν ήταν leader κάπου
    led = Team.query.filter_by(leader_id=u.id).first()
    if led:
        led.leader_id = None
    db.session.delete(u)
    db.session.commit()
...
def admin_reset_password(user_id):
    ...
    db.session.commit()
    flash(f"Προσωρινός κωδικός για {u.username}: {tmp}", "warning")
    return redirect(url_for("admin"))
...
def admin_create_task():
    ...
    t = Task(
        ...
        due_date=due_date_s, due_time=due_time_s,
...
# Admin: Notes edit/delete (διόρθωση σημειώσεων χρηστών)
...
def directory():
    u = current_user()
    is_leader = (u.team is not None and u.team.leader_id == u.id)
...
# (Στο Render: gunicorn app:app --chdir path_to_folder)
